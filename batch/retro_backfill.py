...
- backfill では Discord へ投稿しない（post=False）＝サマリチャンネルへの大量連投を防ぎ、Mongo保存のみ。
...
def main():
    ...
    print(f"[backfill] {start.isoformat()} から過去 {DAYS} 日分を遡及生成（日間 {SLEEP_SEC}s スリープ）")
    ...
    for i in range(DAYS):
        ...
        try:
            # backfill は投稿せず Mongo 保存のみ（31連投スパム防止）。既存日はスキップ。
            status = R.run_for_date(d, client_ai, col, skip_existing=True, post=False)
